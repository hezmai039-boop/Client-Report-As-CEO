...
export async function sendToClient(recipient: string, subject: string, html: string): Promise<MailResult> {
  if (!env.externalSendApproved) {
    await deliver(
      env.ownerEmail,
      `[معلَّق بانتظار الاعتماد] ${subject}`,
  ...
  await deliver(recipient, subject, html);
