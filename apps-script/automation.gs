...
/**
 * يطبّع قيمة حقل رقمي:
 *   "لا يوجد" / "صفر" / "-"  → 0   (نفي صريح، قيمته صفر فعلاً)
 *   "٧٠٠٠ ريال"              → 7000
 *   نص غير رقمي أو فارغ       → ''  (خلية فارغة)
 *
 * إرجاع '' للنص غير الرقمي مقصود: إبقاؤه كنص خام داخل عمود رقمي
 * يُنتج NaN في مجاميع التقارير فيُفسد التقرير كله. النص الأصلي لا
 * يضيع — يبقى محفوظاً كما وصل في تبويب "استقبال واتساب" للمراجعة.
 */
function normalizeNumericAnswer_(value) {
  var v = String(value || '').trim();
  if (!v) return '';
  if (/^(لا\s*يوجد|لايوجد|صفر|بدون|لا\s*شيء|—|–|-|_|\.)$/.test(v)) return 0;
  var n = v.replace(/[^\d.\-]/g, '');
...
function parseNumberedWhatsappReply_(text) {
  ...
  var lastNum = null;   // آخر سؤال أُجيب فعلاً — لاستقبال سطر تكميلي
  var afterBlank = true; // هل السطر الحالي مسبوق بسطر فارغ؟

  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (!line.trim()) { afterBlank = true; continue; }

    var isolated = afterBlank; // نلتقطها قبل إعادة الضبط
    afterBlank = false;

    if (DATE_ONLY.test(line)) { lastNum = null; continue; } // سطر التاريخ ليس إجابة
    ...
    // سطر تكميلي: العميل أضاف بنداً ثانياً تحت إجابة سابقة مباشرة،
    // مثل صنفين مبيعاً في سطرين. الشرط "ملاصق بلا سطر فارغ" هو ما
    // يمنع ابتلاع عبارات الختام ("شاكرين لك") التي تأتي بعد فراغ.
    if (lastNum !== null && !isolated) {
      answers[lastNum] = answers[lastNum] + ' / ' + line.trim();
    }
  }
  return answers;
}
...
function quickAddWhatsappReply() {
  ...
  // ردود واتساب الفعلية كما وصلت. لا تحذف شيئاً من رد العميل —
  // المحلّل يتجاوز الترحيب والختام والأسطر الفارغة بنفسه.
  var replies = [
`حيّاك الله أبو بكر
...
1️⃣ إجمالي مبيعات اليوم (ريال): ٤٥٠٠
2️⃣ عدد الطلبات/الفواتير: ٢٨
3️⃣ تكلفة البضاعة والتشغيل: ٣٥٠٠
4️⃣ عملاء جدد: ٢٠
5️⃣ عملاء متكررون: ٨
...
7️⃣ تقييم رضا العملاء (من 1 إلى 5): ٣.٥

8️⃣ أبرز صنف مبيعاً اليوم:
مضغوط لحم حاشي
...
1️⃣ إجمالي مبيعات اليوم (ريال): ٤٠٠٠
2️⃣ عدد الطلبات/الفواتير: ٢٥
3️⃣ تكلفة البضاعة والتشغيل: ٣٠٠٠
4️⃣ عملاء جدد: ١٥
...
1️⃣ إجمالي مبيعات اليوم (ريال): ٥٠٠٠
2️⃣ عدد الطلبات/الفواتير: ٢٥
3️⃣ تكلفة البضاعة والتشغيل: ٤٠٠٠
4️⃣ عملاء جدد: ١٠
5️⃣ عملاء متكررون: ١٥
...
1️⃣ إجمالي مبيعات اليوم (ريال): ٦٠٠٠
2️⃣ عدد الطلبات/الفواتير: ٣٠
3️⃣ تكلفة البضاعة والتشغيل: ٥٠٠٠
4️⃣ عملاء جدد: ٢٠
5️⃣ عملاء متكررون: ١٠
...
7️⃣ تقييم رضا العملاء (من 1 إلى 5): ٤.٥
